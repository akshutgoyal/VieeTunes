...
export default function NowPlayingSidebar({ open = false, onToggleQueue }) {
  ...
  return <aside className={`now-panel ${open ? 'mobile-visible' : ''}`}>
    <div className="panel-heading sidebar-sticky-heading"><span className="marquee-title">18 Shades of Viee</span><button aria-label="More options">•••</button></div>
    <div className="hero-card"><img src={track.cover} alt=""/><div className="hero-scrim"/><div className="hero-copy"><h2>{track.title}</h2><p>{track.artist} <span className="verified">✓</span></p>{track.description && <p className="track-description">{track.description}</p>}</div></div>
    <section className="artist-info"><div className="artist-info-heading"><strong>About the artist</strong><button>Follow</button></div><p>{track.artist} brings a distinctive atmosphere to every listen. Discover more from this artist in your personal rotation.</p><span className="listeners">One forever listener ♾️</span></section>
